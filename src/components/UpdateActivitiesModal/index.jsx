...
const UpdateActivitiesModal = ({ isOpen, onClose, category, onCategoryUpdated }) => {
  ...
  const handleUpdateCategory = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://sport-reservation-api-bootcamp.do.dibimbing.id/api/v1/sport-categories/update/${category.id}`,
        { name: categoryName },
        {
          headers: {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div className="w-full max-w-sm p-6 bg-white rounded-lg shadow-lg">
        ...
        />
        {error && <p className="mt-2 text-indigo-500">{error}</p>}
